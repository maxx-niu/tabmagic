...
const TabMeasureAdjust: FC<TabDisplayProps> = ({ tabImages, onUploadAgain }) => {
  ...
  const deleteBox = (id: string, pageIndex: number) => {
    setBoundingBoxes((prevBoxes) =>
      prevBoxes.map((boxes, idx) => (idx === pageIndex ? boxes.filter((box) => box.id !== id) : boxes))
    );
  };
  ...
  const addBox = (newBox: number[], pageNumber: number) => {
    ...
    setBoundingBoxes((prevBoxes) =>
      prevBoxes.map((box, index) =>
        index === pageNumber ? [...box, { id: newId, x1: newBox[0], y1: newBox[1], x2: newBox[2], y2: newBox[3] }] : box
      )
    );
    setFlashId(newId);
  ...
  return (
    <>
      <div className="tab-measure-adjust-container">
        ...
        <p style={{ margin: 0 }}>Adjust and add bounding boxes to ensure each bar is properly encapsulated</p>
        <div className="tab-measure-adjust">
          ...
          >
            <button
              onClick={() =>
                addBox(
                  [tabImages[currentPage].image_width - 200, 0, tabImages[currentPage].image_width, 200],
                  currentPage
                )
              }
            >
            ...
            {boundingBoxes[currentPage]?.map((bb) => {
              const scaledBox = [bb.x1, bb.y1, bb.x2, bb.y2].map((value) => value * scaleFactor);
              return (
                <BarBoundingBox
                  key={bb.id}
                  box={scaledBox}
